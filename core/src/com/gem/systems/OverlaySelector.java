...
class OverlaySelector {
	final SystemOverlay systemOverlay;
	private final Gem gem;
	private ShapeRenderer shapeRenderer;

	public OverlaySelector(SystemOverlay overlay, ShapeRenderer _ShapeRenderer, Gem ng) {
		systemOverlay = overlay;
		this.shapeRenderer = _ShapeRenderer;
		gem = ng;
	}
	...
	protected boolean OnEntity(Vector3 mouse, Entity ent) {
		...
		if (ent.hasComponent(ComponentSprite.class)) {
			...
			if (onCircle(mouse, point.getPosition(), w > h ? w : h)) {
				return true;
			}
		} else {
	...
	protected void Overlay(Entity ent, Matrix4 _Comb, boolean _Selected) {
		ComponentPoint point = ent.getComponent(ComponentPoint.class);
		Matrix4 onlyPos = new Matrix4(point.getPosition(), new Quaternion(), new Vector3(1, 1, 1));
		Entity parent = ent.getParent();
		shapeRenderer.setProjectionMatrix(new Matrix4(_Comb).mul(onlyPos));
		if (ent.hasComponent(ComponentSprite.class)) {
			ComponentSprite spriteComp = ent.getComponent(ComponentSprite.class);
			Sprite sprite = spriteComp.getSprite();
			float w = sprite.getWidth() / 2 * sprite.getScaleX();
			float h = sprite.getHeight() / 2 * sprite.getScaleY();
			if (_Selected) {
				shapeRenderer.circle(0, 0, w > h ? w : h);
			}
		} else {
			if (_Selected) {
				shapeRenderer.circle(0, 0, 6);
			}
			shapeRenderer.line(-3, -3, 3, 3);
			shapeRenderer.line(-3, 3, 3, -3);
			shapeRenderer.line(-3, -3, 3, 3);
			shapeRenderer.line(-3, 3, 3, -3);
		}
		...
		shapeRenderer.setProjectionMatrix(_Comb);
		if (parent != null) {
			...
			shapeRenderer.line(pos.x, pos.y, pos2.x, pos2.y);
			shapeRenderer.line(pos2.x, pos2.y, pos2.x + dir1.x * 10, pos2.y + dir1.y * 10);
			shapeRenderer.line(pos2.x, pos2.y, pos2.x + dir2.x * 10, pos2.y + dir2.y * 10);
		}
	}
