...
public class Scene {
	...
	public void onTouchUp(float x, float y, int index) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onTouchUp(): [x=" + x + ",y=" + y + ",index="
					+ index + "]");
	}

	public void onTap(float x, float y, int count) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onTap(): [x=" + x + ",y=" + y + ",count="
					+ count + "]");
	}
	...
	public void onPan(float x, float y, float deltax, float deltay) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onPan(): [x=" + x + ",y=" + y + ",deltax="
					+ deltax + ",deltay=" + deltay + "]");
	}

	public void onPanStop(float x, float y) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onPanStop(): [x=" + x + ",y=" + y + "]");
	}

	public void onZoom(float start, float distance) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onZoom(): [start=" + start + ",distance="
					+ distance + "]");
	}
	...
	public void onPinch(Vector2 start1, Vector2 start2, Vector2 final1,
			Vector2 final2) {
		if (Constant.DEBUG_LEVEL == 0)
			Interface.println("onZoom(): [start1=" + start1 + ",start2="
